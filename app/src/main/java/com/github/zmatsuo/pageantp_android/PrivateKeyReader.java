...
class PrivateKeyReader {
     ...
     */
    public PrivateKeyReader(String keyString) {
     ...
     * @throws IOException
     */
    public PrivateKey read() throws IOException {
        ...
        {
            byte[] keyBytes = readKeyMaterial(P1_BEGIN_MARKER, P1_END_MARKER);
        ...
        {
            byte[] keyBytes = readKeyMaterial(P8_BEGIN_MARKER, P8_END_MARKER);
     ...
     * @throws IOException
     */
    private byte[] readKeyMaterial(String beginMarker, String endMarker) throws IOException
    {
        String line = null;
        ...
        while (scanner.hasNextLine())
        {
            ...
            if (line.contains(endMarker)) {
                return Base64.decode(buf.toString(), Base64.DEFAULT);
    ...
    static public RSAPrivateCrtKeySpec getRSAKeySpec(byte[] keyBytes) throws IOException  {
    ...
    static public RSAPrivateCrtKeySpec getRSAKeySpecPKCS8(byte[] keyBytes) throws IOException  {
        ...
        sequence = parser.read();  // version = 0
        if (sequence.getType() != DerParser.INTEGER) {
        ...
        {
            DerParser parser2 = sequence.getParser();
            Asn1Object sequence2 = parser2.read();
            if (sequence2.getType() != DerParser.OBJECT_IDENTIFIER) {
                throw new IOException("Invalid DER: not a sequence");
            }
            ...
            if (!java.util.Arrays.equals(sequence2.getValue(), identifier)) {
                throw new IOException("Invalid DER: not a sequence");
            }
            Asn1Object s3 = parser2.read();
            if (s3.getType() != DerParser.NULL) {
                throw new IOException("Invalid DER: not a sequence");
            }
        }
        sequence = parser.read();
        if (sequence.getType() != DerParser.OCTET_STRING) {
            throw new IOException("Invalid DER: not a sequence");
        }
        byte [] b = sequence.getValue();
        return getRSAKeySpec(b);
    }
}
...
class DerParser {
    ...
    public Asn1Object read() throws IOException {
        ...
        if (tag == NULL && length == 0) {
            ...
            o = new Asn1Object(tag, length, value);
        }
